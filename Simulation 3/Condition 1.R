...
for(l in 1:sim_number){
  ...
  fx <- parameters$fx[[sample(1:10000,1)]]
  fy <- parameters$fy
  Phi <- parameters$Phi[[k]]
  ...
  fit1.b <- stan_glm(Vy1 ~ Vx1 + Vx2 + Vx3 + Vx4 + Vx5 + Vx6 + Vx7 + Vx8 + Vx9 + Vx10 + Vx11 + Vx12,
                     data = dat, prior = normal(location=rep(0,12),scale=rep(1,12)),chains=1,
                     diagnostic_file = file.path(tempdir(), "df.csv"))
  bf[[1]] <- bridge_sampler(fit1.b)
  
  # psych
  warns <- tryCatch(fa.parallel(dat[,1:12]),error=function(e) e, warning=function(w) w)
  ...
  # best bf
  best_bf <- ifelse(bf(bf[[1]],bf[[2]])[1]>1,1,2)
  best_bf <- ifelse(bf(bf[[best_bf]],bf[[3]])[1]>1,best_bf,3)
  best_bf <- ifelse(bf(bf[[best_bf]],bf[[4]])[1]>1,best_bf,4)
...
save(res,file="model_results_condition_1_500.Rdata")
